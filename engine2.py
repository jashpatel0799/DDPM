...
def ddpm_train(ddpm, dataloader, loss_fn, optimizer, device, ddpm_n_steps, min_beta, max_beta):
    ...
    for step, batch in enumerate(dataloader):
        ...
        t = torch.randint(0, ddpm_n_steps, (n,)).to(device)
        ...
        eta_theta = ddpm(sample = noisy_imgs, timestep = t, return_dict = False)

        # Optimizing the MSE between the noise plugged and predicted noise
        loss = loss_fn(eta_theta[0], eta)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    return epoch_loss, ddpm, optimizer
...
def generate_new_image(ddpm, device, ddpm_n_steps, min_beta, max_beta, n_sample = 1, c=3, h = 64, w = 64):
    ...
    with torch.no_grad():
        ...
        #   eta_theta = ddpm.backward(x, time_tensor)
            eta_theta = ddpm(sample = x, timestep = time_tensor.squeeze(), return_dict = False)
            ...
            x = (1 / alpha_t.sqrt()) * (x - (1 - alpha_t) / (1 - alpha_t_bar).sqrt() * eta_theta[0])
